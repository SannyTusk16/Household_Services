...
export default {
  ...
  methods: {
    async fetchUsers() {
      try {
        ...
        // Fetch Professionals
        const professionalsRes = await axios.get("http://127.0.0.1:5000/get_all_professionals", {
        ...
        // Fetch Customers
        const customersRes = await axios.get("http://127.0.0.1:5000/get_all_customers", {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  try {
    ...
    const response = await axios.post(
      `http://127.0.0.1:5000/block_user/${userId}`,
    );
...
.all-users {
  max-width: 1000px;
  margin: 40px auto;  /* Increased margin to create separation */
